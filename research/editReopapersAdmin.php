...
//cheking if dir. to store files is available if not it is being created
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        //header("location: Reopapers.php");
		 echo "<script>";
		echo "alert('Updated Successfully')";
	    echo "location.replace('editReopapersAdmin.php?id=".$id."')";
	    echo "</script>";
		echo "<script>
								setTimeout(function() {
...
if (isset($_GET['id']) && !empty(trim($_GET['id']))) {
    ...
    $sql = "SELECT * FROM papersbyfaculty WHERE srNo = ?";
...
?>
			...
			<main class="flex-1 p-2 bg-gradient-to-br from-purple-200 to-white min-h-screen">
				<!-- PAge Navigatn -->
				<div class="main-content-inner">
						<nav class=" bg-inherit bg-blue-500 w-full" aria-label="breadcrumb">
							<ol class="list-reset flex">
								...
								<li class="text-gray-500">Conference Paper</li>
							</ol>
						</nav>
				</div>
...
<div class="container  mx-auto p-6 max-w-7xl bg-white rounded-lg shadow-lg">
	...
	<form class="space-y-6" method="post" action="editReopapersAdmin.php" enctype="multipart/form-data">
		<!-- Section 1: Basic Details -->
		<div id="section1" class="form-section active ">
		<input type="hidden" name="university" value="<?php echo $row['University'] ?>">
			...
			<input type="hidden" name="otherauthor" value="<?php echo $row['other Author'] ?>">

			<h4 class="text-lg font-semibold text-gray-900">Basic Details</h4>
			<hr class="w-full max-w-xs mb-4">

			<div class=" my-10">
				<label for="coauthor" class="block text-sm font-medium text-gray-700">Corresponding author's Name<span class="text-red-500">*</span></label>
				<input required type="text" id="coauthor" name="coauthor" value="<?php echo $row['Co-author']; ?>" placeholder="Enter Corresponding author's Name" class="mt-1 w-full max-w-md px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
			</div>

			<div class="my-10">
				<label for="booktitle" class="block text-sm font-medium text-gray-700">Title of Paper<span class="text-red-500">*</span></label>
				<input required type="text" name="booktitle" id="booktitle" value="<?php echo $row['booktitle']; ?>" placeholder="Enter Title of Paper" class="mt-1 w-full max-w-md px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
			</div>

			<div class="my-10">
				<label for="journalname" class="block text-sm font-medium text-gray-700">Name of Journal<span class="text-red-500">*</span></label>
				<input required type="text" name="journalname" id="journalname" value="<?php echo $row['journalname']; ?>" placeholder="Enter Name of Journal" class="mt-1 w-full max-w-md px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
			</div>
		...
		<div id="section2" class="form-section">
			<h4 class="text-lg font-semibold text-gray-900">Paper/Article Submission</h4>
			<hr class="w-full max-w-xs mb-4">
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<!-- Name of conference -->
				<div class="mb-4">
					<label for="conferenceName" class="block text-sm font-medium text-gray-700">Name of conference<span class="text-red-500">*</span></label>
					<input required type="text" name="conferenceName" id="conferenceName" value="<?php echo $row['conferenceName']; ?>" placeholder="Enter Research confrence" class="mt-1 w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
				</div>
				<!-- Institutional Affiliations -->
				<div class="mb-4">
					<label for="conferencePaper" class="block text-sm font-medium text-gray-700">Conference Paper<span class="text-red-500">*</span></label>
					<input required type="text" name="conferencePaper" id="conferencePaper" value="<?php echo $row['conferencePaper']; ?>" placeholder="Enter Conference Paper" class="mt-1 w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
				</div>
				<!-- Publisher Name -->
				<div class="mb-4">
			...
			<h4 class="text-lg font-semibold">Evidence (Upload)</h4>
			<hr class="w-full max-w-xs mb-4">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				...
				<div class="mb-4">
					...
					<input required type="text" name="impact" id="impact-factor" placeholder="Enter Impact Factor"  class="mt-1 w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
				</div>
				</div>
				...
				<div class="mb-6">
					...
					<div class="flex items-center">
						<input required class="form-control date-picker w-full max-w-md px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" id="publication-date" name="publicationdate" type="date" value="<?php echo $row['pubdate']; ?>" data-date-format="yyyy-mm-dd" />
					</div>
				</div>
				<div class="mb-6">
