...
//cheking if dir. to store files is available if not it is being created
    ...
    if ($conn->query($sql) === TRUE) {
	    ...
	    echo "location.replace('editReoBooksAdmin.php?id=".$id."')";
	    echo "</script>";
		echo "<script>
								setTimeout(function() {
...
<div class="container  mx-auto p-6 max-w-7xl bg-white rounded-lg shadow-lg">
	...
	<form class="space-y-6" method="post" action="editReoBooksAdmin.php" enctype="multipart/form-data">
		...
		<input type="hidden" name="author_name" value="<?php echo $row['Faculty'] ?>">
    ...
    <input type="hidden" name="emp_id" value="<?php echo $row['Employee ID']; ?>">
			...
			<div class="my-10">
				<label for="booktitle" class="block text-sm font-medium text-gray-700">Title of Book<span class="text-red-500">*</span></label>
				<input required type="text" name="booktitle" id="booktitle" value="<?php echo $row['booktitle']; ?>" placeholder="Enter Title of Book" class="mt-1 w-full max-w-md px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
			</div>
		...
		<div id="section2" class="form-section">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				...
				<div class="mb-4">
					...
					<input required type="text" name="pubname" id="pubname" value="<?php echo $row['pubname']; ?>" placeholder="Enter Name of Publisher" class="mt-1 w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
				</div>
