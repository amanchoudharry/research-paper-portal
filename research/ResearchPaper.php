    ...
    (`University`, `Department`, `Faculty`, `Employee ID`, `Author`, `Co-author`, `papertitle`, `journalname`, `article`, `region`, `pubdate`, `pubyear`, `volume`, `pagefrom`, `pageto`, `impact`, `scopus`, `listedin`, `wos`, `peer`, `issn`, `isbn`, `pubname`, `affltn`, `corrauthor`, `citind`, `nocit`, `link`, `evdupload`, `othrinfo`, `ref`) 
    VALUES ('$uni', '$department', '$faculty', '$empid', '$author', '$coauthor', '$booktitle', '$journalname', '$article', '$National', '$publicationdate', '$pubyear', '$edition', '$pagefrom', '$pageto', '$impact', '$scopus', '$listedin', '$wos', '$peer', '$issn', '$isbn', '$pubname', '$affltn', '$corrauthor', '$citind', '$nocit', '$link', '$destination', '$othrinfo', '$ref')";
	...
	<form class="space-y-6" method="post" action="ResearchPaper.php" enctype="multipart/form-data" onsubmit="return validateForm();">
		...
		<div id="section1" class="form-section active ">
			...
			<div class="my-10">
				...
				<input required type="text" name="paper_title" id="paper-title" placeholder="Enter Title of Paper" class="mt-1 w-full max-w-md px-4 py-2 border rounded-md border-blue-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
			</div>
    ...
    <input type="url" name="link" id="ugc-recognition-link" placeholder="Enter UGC Recognition Link" class="mt-1 w-full max-w-sm px-4 py-2 border rounded-md border-blue-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" pattern="https?://.+" />
    <!-- <script>
        document.getElementById('ugc-recognition-link').addEventListener('input', function() {
            const urlField = this;
            const urlPattern = /^https?:\/\/.+/;
            
            // Custom validation message for non-empty URLs
            if (urlField.value && !urlPattern.test(urlField.value)) {
                urlField.setCustomValidity('Please enter a valid URL (starting with http:// or https://)');
            } else {
    ...
    if (validateSection(currentFormSection)) {
        const urlField = document.getElementById('ugc-recognition-link');
        const urlPattern = /^https?:\/\/.+/;

        // Validate UGC Recognition Link field
        if (urlField.value && !urlPattern.test(urlField.value)) {
